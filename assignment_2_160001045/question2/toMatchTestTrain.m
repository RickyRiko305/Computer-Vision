function [matchingScore] = toMatchTestTrain(picture1,picture2,method)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% choose the value for method: Cosine_correlation=1; Chi_square=2; Intersection=3; Bhattacharyya_distance=4;
...
if a1*b1 > a2*b2
     ...
     picture2=imresize(picture2,[a1 b1],'bicubic');
...
case 1
    ...
    B=sqrt(sum(hist2.^2));
    C=sum(hist1.*hist2);
    matchingScore=1-C/(A*B);
case 2
    %calculate Chi-Square
    %smaller value, more similar
    A=(hist1-hist2).^2;
    matchingScore=sum(A/hist1);
case 3
...
case 4
    ...
    matchingScore=sqrt(1-A);
% write the code for other distance mesearues
case 5 
    %calculating L1
    hist=abs(hist1-hist2);
    matchingScore=sum(hist);
case 6 
    %calculating L2
    hist=(hist1-hist2);
    hist=(hist.^2);
    matchingScore=sqrt(sum(hist));
case 7
    %calculating L-inf
    hist=abs(hist1-hist2);
    matchingScore=min(hist);
case 8
    %calculating qudratic
    siz = N;
    a = zeros(siz,siz);
    for i=1:N
        for j=1:N
            a(i,j) = 1 - abs(i-j)/(siz-1);
        end
    end
    b = hist1 - hist2;
    matchingScore = sqrt(b*a*(b'));
case 9 
    %calculating normalized euclidean distance
    hist1=(hist1-mean(hist1))/std(hist1);
    hist2=(hist2-mean(hist2))/std(hist2);
    hist=(hist1-hist2);
    hist=(hist.^2);
    matchingScore=sqrt(sum(hist));
end
